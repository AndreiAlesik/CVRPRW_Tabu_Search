...
bool Permutation(vector<Customer>& CustomersVector, vector<Track>& Primary, long double** Matrix)
{
    if (Primary.empty())  // Check if Primary vector is not empty
        return false;

    Track TempTruck(Primary[0].Capacity);

    int vehicle_n0 = random(0, (int)(Primary.size() - 1));
    if (vehicle_n0 >= Primary.size()) // Check if the random index is within the bounds
        return false;

    int vehicle_route_size = Primary[vehicle_n0].Route.size();
    if (vehicle_route_size <= 1) // Make sure there's more than just one node in the route
        return false;

    int insert_node_n0 = random(1, vehicle_route_size - 1);
    if (insert_node_n0 >= vehicle_route_size) // Again, check if the random index is within the bounds
        return false;
    ...
    {
        bool skip_iteration = false;  // Add a flag to skip iteration

        if (i == vehicle_n0 || i >= Primary.size()) // Avoid invalid access and self-insertion
            continue;

        if (track_total_capacity(CustomersVector, Primary[i]) + CustomersVector[insert_node_CID].Demand > Primary[i].Capacity)
            continue;

        Track localTempTruck = TempTruck;
        localTempTruck.Route.clear();
        localTempTruck.Route.push_back(CustomersVector[0]);
        bool pushed = false;
        long double PossibleTime, CurrentTime = 0;
        int CurrentPosition, PreviousPosition = 0;
        for(int j = 1; j < Primary[i].Route.size() && !skip_iteration; j++) // Check flag in loop condition
        {
            if (j >= Primary[i].Route.size()) // Avoid invalid access
            {
                skip_iteration = true;
                continue;
            }
            ...
            {
                localTempTruck.Route.push_back(CustomersVector[insert_node_CID]);
                pushed = true;
            ...
            localTempTruck.Route.push_back(Primary[i].Route[j]);

            PreviousPosition = CurrentPosition;
        }
        if (pushed && validate_route(CustomersVector, localTempTruck, Matrix) && !skip_iteration)
        {
            #pragma omp critical
            {
                if(i < Primary.size()) // Ensure that i index is valid
                {
                    Primary[i].Route.clear();
                    for (int a = 0; a < localTempTruck.Route.size(); a++)
                        Primary[i].Route.push_back(localTempTruck.Route[a]);
                }

                if (vehicle_route_size == 2 && vehicle_n0 < Primary.size()) // Ensure that vehicle_n0 index is valid
                    Primary.erase(Primary.begin() + vehicle_n0);
                else if(insert_node_n0 < Primary[vehicle_n0].Route.size()) // Ensure that insert_node_n0 index is valid
